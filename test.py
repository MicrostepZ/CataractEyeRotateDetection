...
class PointTracker(object):
  ...
  ##计算向量、旋转部分
  def array_rotate_compute(self, pre_points, last_points, pre_center, last_center, rotate_truth, filter_thresh):  # 计算两张图之间的旋转角度
    ...
    ret_total = np.sum(retain)
    ls_05 = np.sum(abs(ls - rotate_truth) < 0.5) / ret_total
    ls_1 = np.sum(abs(ls - rotate_truth) < 1) / ret_total
    ls_2 = np.sum(abs(ls - rotate_truth) < 2) / ret_total
    ls_5 = np.sum(abs(ls - rotate_truth) < 5) / ret_total
    if np.sum(retain) == 0:
      return 0,0,0,0,0,0,0
    else:
      return np.median(ls), total, ls_05, ls_1, ls_2, ls_5, ret_total

  def draw_tracks(self, image0, image1, tracks, oval0, oval1, rotate_truth, filter_thresh):

    image0 = cv2.cvtColor(np.array(image0, dtype='uint8'), cv2.COLOR_BGR2GRAY)
    image1 = cv2.cvtColor(np.array(image1, dtype='uint8'), cv2.COLOR_BGR2GRAY)
    margin = 20
    H0, W0 = image0.shape
    H1, W1 = image1.shape
    ...
    for track in tracks:
      ...
      clr = self.sigle_rotate_compute(p0, p1, pre_center=(oval0[0], oval0[1]), last_center=(oval1[0], oval1[1]),
                                      rotate_truth=rotate_truth)
      cv2.line(out, p0, (int(round(pt1[0])+W0+margin), int(round(pt1[1]))),  clr , thickness=stroke, lineType=16)
      cv2.circle(out, p0, stroke, (0, 255, 255), -1, lineType=16)
      cv2.circle(out, (int(round(pt1[0])+W0+margin), int(round(pt1[1]))), stroke, (0, 255, 255), -1, lineType=16)
      self.point_list_pre.append(p0)
      self.point_list_last.append(p1)
    rotate, total, ls_05, ls_1, ls_2, ls_5, ret_total = self.array_rotate_compute(self.point_list_pre, self.point_list_last, pre_center=(oval0[0], oval0[1]), last_center=(oval1[0], oval1[1]), rotate_truth=rotate_truth,filter_thresh=filter_thresh)
    print('Rotate:{:.5f}'.format(rotate))
    return rotate, out, total, ls_05, ls_1, ls_2,ls_5,ret_total
...
if __name__ == '__main__':
  ...
  parser.add_argument('--eval_result', type=str, default='output/predict/MMA_MMP_MVMP.txt', help='Directory where to write output txt.')
  ...
  parser.add_argument('--no_display', action='store_true', help='Do not display images to screen. Useful if running remotely (default: False).')
  parser.add_argument('--write',type=bool, default=True, help='Save output frames to a directory (default: False)')
  ...
  for vid in vid_list:
    ...
    for seq in seq_list:
      ...
      root_t = 'data/Cataract_Test/Rotate_Index_Standard/UC'
      rotate_truth_dict = getRotateDict(os.path.join(root_t, vid, seq + '.txt'))
      ...
      if opt.write:
        print('==> Will write outputs to %s' % save_image_path)
      ...
      while True:
        ...
        ## 眼球旋转角度计算(包括异常匹配点剔除)
        rotate_truth = float(rotate_truth_dict[image_id])
        rotate, image_display, total, ls_05, ls_1, ls_2, ls_5, ret_total = tracker.draw_tracks(image0=image0, image1=image, tracks=tracks, oval0=oval0, oval1=oval1, rotate_truth=rotate_truth, filter_thresh=opt.filter_thresh)
        file.writelines(image_id+':'+str(rotate)+'\n')

        ## 更新相关指标
        total_MMP += total
        total_MVMP += ret_total
        total_05 += ls_05
        total_10 += ls_1
        total_20 += ls_2
        total_50 += ls_5
        ...
        if not opt.no_display:
          cv2.imshow(win, image_display)
          key = cv2.waitKey(opt.waitkey) & 0xFF


        # Optionally write images to disk.
        if opt.write:
          out_file = os.path.join(save_image_path, image_id)
          print('Writing image to %s' % out_file)
          cv2.imwrite(out_file, image_display)
  ...
  MMP = round(total_MMP / time_i)
  MVMP = round(total_MVMP / time_i)
  MMA_05 = round(total_05 / time_i,3)
  MMA_10 = round(total_10 / time_i,3)
  MMA_20 = round(total_20 / time_i,3)
  MMA_50 = round(total_50 / time_i,3)

  print('MMA_@0.5:{:.3f}, @1.0:{:.3f}, @2.0:{:.3f}, @5.0:{:.3f}, MVMP:{}, MVP:{}'.format(MMA_05, MMA_10, MMA_20, MMA_50, MVMP, MMP))
  FPS_file = open(os.path.join(opt.eval_result), 'w+')
  FPS_file.writelines('MMA@0.5:' + str(MMA_05) + '\n')
  FPS_file.writelines('MMA@1.0:' + str(MMA_10) + '\n')
  FPS_file.writelines('MMA@2.0:' + str(MMA_20) + '\n')
  FPS_file.writelines('MMA@5.0:' + str(MMA_50) + '\n')
  FPS_file.writelines('MVMP:' + str(MVMP) + '\n')
  FPS_file.writelines('MMP:' + str(MMP) + '\n')
  FPS_file.close()
